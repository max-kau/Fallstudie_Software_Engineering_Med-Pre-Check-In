    ...
    <div class="view">
      ...
      <div class="container container--form">
        <div class="view-content">
          <h2 style="margin-bottom:var(--space-2)">${t('allergien.title')}</h2>
          <p class="text-muted" style="margin-bottom:var(--space-6)">${t('allergien.subtitle')}</p>
          <div id="allergien-input-area" style="${data.keine ? 'opacity:0.4;pointer-events:none' : ''}">
            <div class="form-group" style="margin-bottom:var(--space-6)">
              <label class="form-label">${t('allergien.search_placeholder')}</label>
              ${renderTagInput('allergien', data.liste, t('allergien.search_placeholder'), COMMON_ALLERGIES)}
            </div>
            <div class="form-group">
              <label class="form-label" for="allergien-anmerkungen">${t('allergien.notes_label')} <span style="font-weight:400;color:var(--gray-400)">${t('allergien.optional')}</span></label>
              <textarea class="form-textarea" id="allergien-anmerkungen" placeholder="..." style="min-height:80px">${data.anmerkungen}</textarea>
            </div>
          </div>
          <label class="no-data-toggle ${data.keine ? 'active' : ''}" id="keine-allergien-toggle">
            ...
            <span class="no-data-text">${t('allergien.no')}</span>
          </label>
          ${renderStepNavigation('medikamente', 'ai-fragen')}
        </div>
      </div>
    </div>`;
}
...
function validateAllergien() {
  ...
  if (!data.keine && data.liste.length === 0) {
    return { valid: false, message: t('allergien.val_required') };
  }
  return { valid: true };
}
...
export function initAllergienView() {
  ...
  function reRenderTags() {
    ...
    area.innerHTML = `<div class="form-group" style="margin-bottom:var(--space-6)"><label class="form-label">${t('allergien.add_label')}</label>${renderTagInput('allergien', data.liste, t('allergien.input_placeholder'), COMMON_ALLERGIES)}</div><div class="form-group"><label class="form-label" for="allergien-anmerkungen">${t('allergien.notes_praxis_label')} <span style="font-weight:400;color:var(--gray-400)">${t('allergien.optional')}</span></label><textarea class="form-textarea" id="allergien-anmerkungen" placeholder="${t('allergien.notes_placeholder')}" style="min-height:80px">${anmVal}</textarea></div>`;
